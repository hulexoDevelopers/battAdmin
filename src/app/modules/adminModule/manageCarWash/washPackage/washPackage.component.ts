...
@Component({
  ...
  styleUrls: ['./washPackage.component.scss'],
})
export class washPackageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.myForm = this.fb.group({
      ...
      packageList: this.fb1.array([], [Validators.required])
    });
  ...
  get packageList() {
    return this.myForm.get("packageList") as FormArray;
  }
  ...
  getAllPackages() {
    this.packageService.getAllpackages().subscribe(res => {
      ...
      this.packageListData = res[0].packageList;
      this.data();
    })
  }
  ...
  private data() {
    ...
    this.serverPackageData = this.packageListData;
    ...
    this.serverPackageData.map(e =>
      this.packageList.push(this.fb1.group({ id: e.packageId, title: e.title, price: e.price }))
    )
  }
