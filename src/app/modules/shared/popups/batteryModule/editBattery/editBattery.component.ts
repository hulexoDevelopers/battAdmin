...
@Component({
  selector: 'app-editBattery',
  templateUrl: './editBattery.component.html',
  styleUrls: ['./editBattery.component.scss']
})
export class editBatteryComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<editBatteryComponent>,
  ...
  getBatteryById() {
    this.batteryService.getBatteryById(this.batteryId).subscribe(res => {
      ...
      this.battery.price = res.data.price;
      this.battery.warranty = res.data.warranty;
      if (res.data.companyId) {
        let company = this.allCompanies.find(data => data._id == res.data.companyId);
        if (company) {
          this.battery.companyId = res.data.companyId;
